...
def set_other_mpi_vars(environ_cp):
  ...
  if os.path.exists(os.path.join(mpi_home, 'include/mpi_portable_platform.h')):
    ...
    symlink_force(
        os.path.join(mpi_home, 'include/cray_version.h'), 'third_party/mpi/cray_version.h')
    symlink_force(
        os.path.join(mpi_home, 'include/mpicxx.h'), 'third_party/mpi/mpicxx.h')
    symlink_force(
        os.path.join(mpi_home, 'include/mpio.h'), 'third_party/mpi_collectives/mpio.h')
    symlink_force(
        os.path.join(mpi_home, 'include/cray_version.h'), 'third_party/mpi_collectives/cray_version.h')
    symlink_force(
        os.path.join(mpi_home, 'include/mpicxx.h'), 'third_party/mpi_collectives/mpicxx.h')

    #symlink_force(
    #    os.path.join(mpi_home, 'include/mpi_utils.h'), 'third_party/mpi/mpi_utils.h')
    # TODO(gunan): avoid editing files in configure
    sed_in_place('third_party/mpi/mpi.bzl', 'MPI_LIB_IS_OPENMPI=True',
                 'MPI_LIB_IS_OPENMPI=False')

  if os.path.exists(os.path.join(mpi_home, 'lib/libmpich.so')):
    symlink_force(
        os.path.join(mpi_home, 'lib/libmpich.so'), 'third_party/mpi/libmpi.so')
  elif os.path.exists(os.path.join(mpi_home, 'lib/libmpi.so')):
    symlink_force(
